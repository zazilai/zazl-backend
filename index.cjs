...
const axios = require('axios'); // For direct xAI API calls
...
app.post('/twilio-whatsapp', loggerMw(db), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Use OpenAI for intent (fallback from Grok if needed)
    ...
    try {
      // Try Grok 4 via xAI API, timeout after 5s
      const xaiPromise = axios.post('https://api.x.ai/v1/chat/completions', {
        model: 'grok-4',
        temperature: 0.3,
        max_tokens: 2048,
        messages: [
      ...
      }).then(r => r.data.choices[0].message.content || null);

      const xaiAnswer = await Promise.race([
        xaiPromise,
        new Promise(resolve => setTimeout(() => resolve(null), 5000))
      ]);

      if (xaiAnswer) {
        mainAnswer = xaiAnswer;
        usedModel = 'Grok 4';
      } else {
