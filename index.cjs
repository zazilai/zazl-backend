...
// Helpers & Services (implement each in /helpers or /services)
...
// Public FX JSON endpoint (for future web app)
...
app.post('/twilio-whatsapp', loggerMw(db), async (req, res) => {
  ...
  try {
    // Pull (or create) user profile – contains plan, locale, etc.
    const userProfile = await profileSvc.load(db, waNumber);

    // 1️⃣ Basic intent detection (OpenAI function-calling)
    ...
    // Persist usage (for quotas / analytics)
    await profileSvc.updateUsage(db, waNumber, replyObj.tokens || 0);

    // Twilio expects XML <Response><Message>…</Message></Response>
    res.type('text/xml');
